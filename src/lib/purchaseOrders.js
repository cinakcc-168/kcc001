...
export async function loadPurchaseOrderWorkspace(supabase, profile, filters) {
  ...
  const [supplierResult, productResult, purchaseResult] = await Promise.all([
    supabase
      .from("suppliers")
      .select("id,supplier_code,name,contact_name,phone,email,address,tax_id,notes,is_active,created_at,updated_at")
  ...
  for (const result of [supplierResult, productResult, purchaseResult]) {
  ...
  const purchases = (purchaseResult.data || []).map((purchase) => ({
    ...
    purchase_items: [...(purchase.purchase_items || [])].sort((a, b) =>
      String(a.products?.name || "").localeCompare(String(b.products?.name || ""))
    ),
    purchase_payments: [...(purchase.purchase_payments || [])].sort(
...
export async function savePurchaseOrder(supabase, values) {
  const { data, error } = await supabase.rpc("save_purchase_order", {
    ...
    p_payment_terms: values.payment_terms?.trim() || null,
    p_delivery_address: values.delivery_address?.trim() || null,
...
export async function cancelPurchaseOrder(supabase, purchaseId, reason) {

...
import { Activity, CalendarClock, Edit3, Gift, MessageCircle, Plus, RefreshCw, Search, Send, Tags, UserRoundCheck, UsersRound } from "lucide-react";
...
export default function CustomerCrmPage() {
  ...
  return (
    <div className="page-stack crm-page">
      ...
      {tab === "campaigns" && <section className="panel"><div className="section-heading"><div><h2>Telegram customer campaigns</h2><p>Campaigns are delivered only to linked, opted-in customers.</p></div><button className="primary-button" onClick={() => { setEditingCampaign(null); setCampaignOpen(true); }}><Plus size={18} />New campaign</button></div><div className="responsive-table"><table><thead><tr><th>Campaign</th><th>Type</th><th>Segment</th><th>Scheduled</th><th>Delivery</th><th>Status</th><th /></tr></thead><tbody>{setup.campaigns.map((c) => <tr key={c.id}><td><strong>{c.name}</strong><small>{c.title_en}</small></td><td>{c.campaign_type.replace("_"," ")}</td><td>{c.crm_segments?.name || "All opted-in"}</td><td>{dateTime(c.scheduled_at)}</td><td>{Number(c.sent_count || 0).toLocaleString("en-US")} sent · {Number(c.failed_count || 0).toLocaleString("en-US")} failed</td><td><span className={`status-pill ${c.status}`}>{c.status}</span></td><td><div className="table-actions"><button onClick={() => { setEditingCampaign(c); setCampaignOpen(true); }}>Edit</button>{["draft","scheduled"].includes(c.status) && <button onClick={() => run(() => dispatchCustomerCampaign(session, c.id), "Campaign dispatch started.")}><Send size={17} /></button>}</div></td></tr>)}</tbody></table></div><div className="crm-campaign-mobile-list">{setup.campaigns.map((c) => (<article className="crm-campaign-mobile-card" key={`mobile-camp-${c.id}`}><header><div><strong>{c.name}</strong>{c.title_en && <small>{c.title_en}</small>}</div><span className={`status-pill ${c.status}`}>{c.status}</span></header><div className="crm-customer-mobile-grid"><div><span>Type</span><strong>{c.campaign_type.replace("_", " ")}</strong></div><div><span>Segment</span><strong>{c.crm_segments?.name || "All opted-in"}</strong></div><div><span>Scheduled</span><strong>{dateTime(c.scheduled_at)}</strong></div><div><span>Delivery</span><strong>{Number(c.sent_count || 0).toLocaleString("en-US")} sent · {Number(c.failed_count || 0).toLocaleString("en-US")} failed</strong></div></div><div className="crm-customer-mobile-actions"><button type="button" className="secondary-button compact-button" onClick={() => { setEditingCampaign(c); setCampaignOpen(true); }}><Edit3 size={16} />Edit</button>{["draft", "scheduled"].includes(c.status) && (<button type="button" className="primary-button compact-button" onClick={() => run(() => dispatchCustomerCampaign(session, c.id), "Campaign dispatch started.")}><Send size={16} />Send now</button>)}</div></article>))}</div></section>}

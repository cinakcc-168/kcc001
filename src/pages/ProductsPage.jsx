              ...
              {pagedProducts.map((product) => {
                ...
                return (
                  <article className="list-record-card product-directory-card" key={product.id}>
                    <header><div className="product-cell"><div className="product-thumb"><MediaImage src={product.image} alt={product.name} width={96} height={96} /></div><div>{(() => { const { primaryName, secondaryName } = productDisplayNames(product); return <><strong className="product-name-primary no-translate" data-i18n-skip title={primaryName}>{primaryName}</strong>{secondaryName && <span className="product-name-secondary no-translate" data-i18n-skip title={secondaryName}>{secondaryName}</span>}<small>{product.sku || "No code"} · {product.unit_name}</small></>; })()}</div></div><span className={`status-pill ${product.is_active ? "active" : "inactive"}`}>{product.is_active ? "Active" : "Inactive"}</span></header>
              ...
              <table className="product-table">
                ...
                <tbody>{pagedProducts.map((product) => {
                  ...
                  return <tr key={product.id}>
                    <td data-label="Product"><div className="product-cell"><div className="product-thumb"><MediaImage src={product.image} alt={product.name} width={96} height={96} /></div><div>{(() => { const { primaryName, secondaryName } = productDisplayNames(product); return <><strong className="product-name-primary no-translate" data-i18n-skip title={primaryName}>{primaryName}</strong>{secondaryName && <span className="product-name-secondary no-translate" data-i18n-skip title={secondaryName}>{secondaryName}</span>}<small>{product.sku || "No code"} · {product.unit_name}</small></>; })()}</div></div></td>

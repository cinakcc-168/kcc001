begin;
-- Step 46.46: Product Promotion online eligibility + base-unit quantity cap.
-- Additive migration after Step 46.45.

alter table public.product_promotions
  add column if not exists allow_online boolean not null default true,
  add column if not exists max_base_quantity numeric(14,3),
  add column if not exists reserved_base_quantity numeric(14,3) not null default 0;

update public.product_promotions
set reserved_base_quantity = greatest(coalesce(reserved_base_quantity,0),0),
    max_base_quantity = case when max_base_quantity is not null and max_base_quantity <= 0 then null else max_base_quantity end;

alter table public.product_promotions
  drop constraint if exists product_promotions_max_base_qty_ck;
alter table public.product_promotions
  add constraint product_promotions_max_base_qty_ck
  check (max_base_quantity is null or max_base_quantity > 0);
alter table public.product_promotions
  drop constraint if exists product_promotions_reserved_base_qty_ck;
alter table public.product_promotions
  add constraint product_promotions_reserved_base_qty_ck
  check (reserved_base_quantity >= 0);
...
create or replace function public.save_product_promotion(
  ...
  p_allow_online boolean default true, p_max_base_quantity numeric default null,
  p_is_active boolean default true, p_notes text default null, p_product_ids uuid[] default '{}'
) returns jsonb language plpgsql security definer
set search_path=public,private,auth,pg_temp as $$
declare v_user uuid:=auth.uid(); v_profile record; v_promotion public.product_promotions%rowtype; v_product_id uuid;
begin
  ...
  if p_max_base_quantity is not null and p_max_base_quantity<=0 then raise exception 'Promotion quantity limit must be greater than zero'; end if;
  ...
  if p_promotion_id is null then
    ...
    values(v_profile.organization_id,p_branch_id,trim(p_name),p_discount_type,round(p_discount_value,4),p_starts_at,p_ends_at,coalesce(p_allow_coupon,true),coalesce(p_allow_manual_discount,true),coalesce(p_allow_online,true),p_max_base_quantity,0,coalesce(p_is_active,true),nullif(trim(p_notes),''),v_user,v_user)
    returning * into v_promotion;
  else
    update public.product_promotions set branch_id=p_branch_id,name=trim(p_name),discount_type=p_discount_type,discount_value=round(p_discount_value,4),starts_at=p_starts_at,ends_at=p_ends_at,allow_coupon=coalesce(p_allow_coupon,true),allow_manual_discount=coalesce(p_allow_manual_discount,true),allow_online=coalesce(p_allow_online,true),max_base_quantity=p_max_base_quantity,updated_by=v_user,updated_at=now(),is_active=coalesce(p_is_active,true),notes=nullif(trim(p_notes),'')
  ...
  foreach v_product_id in array p_product_ids loop
    insert into public.product_promotion_items(organization_id,promotion_id,product_id) values(v_profile.organization_id,v_promotion.id,v_product_id);
  end loop;
  return jsonb_build_object('ok',true,'promotion',to_jsonb(v_promotion));
end $$;
revoke all on function public.save_product_promotion(uuid,text,uuid,public.discount_type,numeric,timestamptz,timestamptz,boolean,boolean,boolean,numeric,boolean,text,uuid[]) from public,anon;
grant execute on function public.save_product_promotion(uuid,text,uuid,public.discount_type,numeric,timestamptz,timestamptz,boolean,boolean,boolean,numeric,boolean,text,uuid[]) to authenticated,service_role;
...
declare v_row record; v_remaining numeric;
begin
  select pp.*,pi.product_id into v_row
  from public.product_promotions pp join public.product_promotion_items pi on pi.promotion_id=pp.id and pi.organization_id=pp.organization_id
  ...
  order by pp.starts_at desc,pp.created_at desc,pp.id desc limit 1;
...
create or replace function private.claim_product_promotion_units_v1(
  p_promotion_id uuid,p_requested_base_quantity numeric,p_unit_factor numeric
) returns jsonb language plpgsql security definer set search_path=public,private,auth,pg_temp as $$
declare v_row public.product_promotions%rowtype; v_remaining numeric; v_grant numeric;
begin
  if p_requested_base_quantity is null or p_requested_base_quantity<=0 then return jsonb_build_object('granted_base_quantity',0); end if;
  select * into v_row from public.product_promotions where id=p_promotion_id for update;
  if not found or not v_row.is_active then return jsonb_build_object('granted_base_quantity',0); end if;
  if v_row.max_base_quantity is null then v_grant:=floor(p_requested_base_quantity/nullif(p_unit_factor,0))*p_unit_factor;
  else v_remaining:=greatest(v_row.max_base_quantity-coalesce(v_row.reserved_base_quantity,0),0); v_grant:=floor(least(p_requested_base_quantity,v_remaining)/nullif(p_unit_factor,0))*p_unit_factor; end if;
  v_grant:=greatest(v_grant,0);
  if v_grant>0 then update public.product_promotions set reserved_base_quantity=round(coalesce(reserved_base_quantity,0)+v_grant,3),updated_at=now() where id=v_row.id; end if;
  return jsonb_build_object('granted_base_quantity',v_grant,'promotion_id',v_row.id,'remaining_base_quantity',case when v_row.max_base_quantity is null then null else greatest(v_row.max_base_quantity-(coalesce(v_row.reserved_base_quantity,0)+v_grant),0) end);
end $$;
revoke all on function private.claim_product_promotion_units_v1(uuid,numeric,numeric) from public;
grant execute on function private.claim_product_promotion_units_v1(uuid,numeric,numeric) to authenticated,service_role;
...
begin
  if new.status='voided' and coalesce(old.status,'')<>new.status then
    for v_row in select si.promotion_id, sum(si.base_quantity) base_qty from public.sale_items si where si.sale_id=new.id and si.promotion_id is not null group by si.promotion_id loop
      update public.product_promotions set reserved_base_quantity=greatest(coalesce(reserved_base_quantity,0)-v_row.base_qty,0),updated_at=now() where id=v_row.promotion_id;
    end loop;
  end if;
  return new;
end $$;
drop trigger if exists release_promotion_reservation_on_sale_void on public.sales;
create trigger release_promotion_reservation_on_sale_void after update of status on public.sales for each row execute function private.release_promotion_reservation_for_sale();
...
begin
  if new.status in ('cancelled','rejected') and old.status is distinct from new.status then
    for v_row in select oi.promotion_id, sum(oi.promotion_base_quantity) base_qty from public.online_order_items oi where oi.online_order_id=new.id and oi.promotion_id is not null group by oi.promotion_id loop
      update public.product_promotions set reserved_base_quantity=greatest(coalesce(reserved_base_quantity,0)-v_row.base_qty,0),updated_at=now() where id=v_row.promotion_id;
    end loop;
  end if;
  return new;
end $$;
drop trigger if exists release_promotion_reservation_on_online_cancel on public.online_orders;
create trigger release_promotion_reservation_on_online_cancel after update of status on public.online_orders for each row execute function private.release_promotion_reservation_for_online_cancel();
...
alter table public.online_order_items
  ...
  add column if not exists promotion_base_quantity numeric(14,3) not null default 0 check(promotion_base_quantity>=0);
create index if not exists online_order_items_promotion_idx on public.online_order_items(promotion_id) where promotion_id is not null;
...
begin
  ...
  loop
    ...
    if v_promotion is not null then
      v_promotion_granted_base := coalesce((private.claim_product_promotion_units_v1(
        (v_promotion ->> 'id')::uuid,
        v_base_quantity,
    ...
    select jsonb_build_object(
      ...
      'units',(
        select coalesce(jsonb_agg(jsonb_build_object(
          ...
          'promotion', private.active_product_promotion_v2(
            v_store.organization_id,v_store.branch_id,p.id,u.id,p.currency,true,now()
          )
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_payload->'items') loop
    ...
    if v_promotion is not null then
      v_promotion_granted_base:=coalesce((private.claim_product_promotion_units_v1((v_promotion->>'id')::uuid,v_base_quantity,v_unit.conversion_factor)->>'granted_base_quantity')::numeric,0);
      v_promotion_units:=least(v_quantity,floor(v_promotion_granted_base/nullif(v_unit.conversion_factor,0)));
      v_promotion_unit_discount:=round(greatest(v_unit_price-case when v_promotion->>'discount_type'='percent' then v_unit_price*(1-least(greatest((v_promotion->>'discount_value')::numeric,0),100)/100) else greatest(v_unit_price-greatest((v_promotion->>'discount_value')::numeric,0),0) end,0),2);
    ...
    insert into public.online_order_items(
      ...
      list_price,unit_price,line_total,promotion_id,promotion_discount_amount,promotion_base_quantity
    ) values(
      ...
      v_promotion_id,v_promotion_discount,round(v_promotion_units*v_unit.conversion_factor,3)
    );
  end loop;

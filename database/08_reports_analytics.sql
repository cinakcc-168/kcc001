...
begin
  ...
  if v_profile.role not in ('owner', 'admin', 'manager', 'viewer') then
    raise exception 'Your role cannot open business reports';
  end if;
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      'bucket', b.bucket,
      'product_count', count(a.product_id),
      'quantity', coalesce(sum(a.quantity), 0),
      'stock_value', coalesce(sum(a.cost_value), 0)
    ) order by b.bucket_order
  ), '[]'::jsonb)
  into v_stock_age
  from buckets b
  left join aged a on a.age_days between b.min_days and b.max_days

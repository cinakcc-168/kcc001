...
create table if not exists public.return_item_batches (
  ...
  sale_item_batch_id uuid not null references public.sale_item_batches(id) on delete restrict,
  inventory_batch_id uuid not null references public.inventory_batches(id) on delete restrict,
  base_quantity numeric(14,3) not null check (base_quantity > 0),
...
set search_path=public,private,auth,pg_temp as $$
declare v_result jsonb; v_return_id uuid; v_ri record; v_alloc record; v_restored numeric(14,3); v_available numeric(14,3);
 v_take numeric(14,3); v_remaining numeric(14,3); v_today date;
begin
  v_result:=public.process_sale_return_v3(p_sale_id,p_items,p_refund_method,p_reason,p_refund_reference,p_approval_request_id);
  v_return_id:=(v_result->>'return_id')::uuid;
  if exists(select 1 from public.return_item_batches where return_item_id in(select id from public.return_items where return_id=v_return_id)) then return v_result; end if;
  v_today:=coalesce(private.batch_business_date(private.current_organization_id()),current_date);
  for v_ri in select ri.*,p.batch_tracking from public.return_items ri join public.products p on p.id=ri.product_id
    where ri.return_id=v_return_id and ri.restock=true and p.batch_tracking=true loop
    v_remaining:=v_ri.base_quantity;
    for v_alloc in select sib.* from public.sale_item_batches sib
      where sib.sale_item_id=v_ri.sale_item_id order by sib.allocation_order,sib.created_at loop
      exit when v_remaining<=0.0005;
      select coalesce(sum(rib.base_quantity),0) into v_restored from public.return_item_batches rib
        where rib.sale_item_batch_id=v_alloc.id and rib.restocked=true;
      ...
      update public.inventory_batches set quantity=quantity+v_take,
        ...
        updated_at=now() where id=v_alloc.inventory_batch_id;
      insert into public.return_item_batches(organization_id,return_item_id,sale_item_batch_id,inventory_batch_id,base_quantity,restocked)
      values(private.current_organization_id(),v_ri.id,v_alloc.id,v_alloc.inventory_batch_id,v_take,true);
      v_remaining:=round(v_remaining-v_take,3);
    end loop;
    if v_remaining>0.0005 then raise exception 'Original batch trace is incomplete for the returned product'; end if;
  end loop;
  return v_result||jsonb_build_object('batch_restocked',true);
end; $$;
